...
export default function Home() {
  ...
  const isItemLoaded = (index: number) => items.current.length > index;
  const loadMoreItems = useCallback(async (startIndex: number, stopIndex: number) => {
    ...
    items.current.push(...data['data'])
    hasNextPage.current = items.current.length >= data['total']
    setCount(items.current.length < data['total'] ? items.current.length + 10 : items.current.length)
  }, []);
